...
3. **Argo CD starts, finds `root`, and reads `apps/`.** It finds two more `Application` objects
   ...
   - `apps/platform.yaml` tells Argo CD to render the chart in `platform/` and apply it. That
     creates the `platform` namespace, the Gateway, and the one object that puts the Config
     Connector add-on into cluster mode acting as the identity `terraform-gcp` created. Google
     sees the Gateway and builds the load balancer, bound to the reserved address; the add-on's
     operator sees its object and starts the controller in `cnrm-system`.
   - `apps/argocd.yaml` tells Argo CD to apply the Argo CD chart at a pinned version with the
     values in `argocd/values.yaml`. The chart and values are exactly what Terraform installed a
     minute earlier, so Argo CD finds its own objects already present and matching, and takes
     ownership of them. The values include Argo CD's route on the Gateway and the load
     balancer's health check for it.
...
| --- | --- |
| `platform.yaml` | The chart in `platform/`, into the `platform` namespace. Has a finalizer so that deleting the Application deletes the Gateway rather than orphaning the load balancer behind it. Server-side apply, because the Config Connector operator creates its object first and owns its fields |
| `argocd.yaml` | Two sources at once: the Argo CD chart at a pinned version from Argo's chart repository, and `argocd/values.yaml` from here as the chart's input. This is Argo CD managing itself |

### `platform/` — what every application shares

A small Helm chart. Its `values.yaml` holds two names, the environment and the project, and
every object it renders derives its names from them the way `terraform-gcp` derives every
resource name from the environment's name: `<environment>-gateway`,
`<environment>-certificates`, `<environment>-config-connector`. Moving the platform to another
environment is editing those two lines. Applied by `apps/platform.yaml`. Rendered without
applying by `helm template platform platform`.

| File | What it is |
| --- | --- |
| `Chart.yaml`, `values.yaml` | The chart's name, and the two values: `environment` and `projectId`. The scripts read the values too |
| `templates/namespace.yaml` | The `platform` namespace |
| `templates/gateway.yaml` | The one Gateway, and so the one load balancer, every application uses. It binds to the reserved address and the certificate map that `terraform-gcp` created, and it admits routes from any namespace labelled `shared-gateway-access: "true"`. No application creates a Gateway; each creates an HTTPRoute in its own namespace that names this one |
| `templates/config-connector.yaml` | The `ConfigConnector` object, name fixed by Google: cluster mode, acting as `<environment>-config-connector`, and `stateIntoSpec: Absent` so Google's defaults never leak back into the objects it reconciles. Config Connector is the controller that turns Kubernetes objects such as `PubSubTopic` into the Google resources they describe, so that an application's chart can carry its own Google resources. `terraform-gcp` switches the add-on on and creates the identity; this object tells it which mode to run in and which identity that is. Every namespace that holds Config Connector objects must carry the annotation `cnrm.cloud.google.com/project-id` naming the project. Why cluster mode, and the fences around a shared identity, are in `terraform-gcp`'s ADR 0003 |
...
| File | What it is |
| --- | --- |
| `values.yaml` | The chart's input: Argo CD's own hostname, the server in the mode that expects TLS terminated in front of it, which optional components are on, resource requests sized for this cluster, and, as extra objects the chart renders, Argo CD's HTTPS route on the shared Gateway, the HTTP-to-HTTPS redirect, the health check the load balancer uses for the Argo CD server, and, when `frontDoor.securityPolicy` names one, the policy on who may reach it. The hostname is written twice in this file and nowhere else in the repository |
...
| `lib/platform.sh` | Sourced by both scripts. Reads the environment name, project, region and state prefix out of `terraform/`, the hostname out of `argocd/values.yaml`, the two values out of `platform/values.yaml`, and the Application names out of `apps/`; derives every other name from the environment's name the way the module and the chart do, and refuses if the files disagree with each other |
